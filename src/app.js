...
const trivoryApi = process.env.TRIVORY_API;
...
async function getPeriodData(school) {
  try {
    const response = await axios.get(
      "https://trivory.com/api/today_schedule?schoolid=" +
        school +
        "&language=en&api_key=" +
        trivoryApi
    );
    ...
    dt = dateTime();
    schedule["time_updated"] = `${dt.date.year.toString().slice(-2)}-${
      dt.date.month
    }-${dt.date.day} ${dt.time.hours}:${dt.time.minutes} ${dt.time.ampm}`;
    ...
    try {
      ...
      console.log(
        chalk.green.dim(
          `${dt.date.year}/${dt.date.month}/${dt.date.day} ${dt.time.hours}:${dt.time.minutes}:${dt.time.seconds} ${dt.time.ampm}` +
            " | " +
            school +
            " schedule updated"
        )
...
function verifyEmail(email, name, verificationKey) {
  ...
  fs.readFile("emails/verifyEmailHtml.html", "utf8", (err, data) => {
    ...
    const modifiedHtml = data
      ...
      .replace(
        "{{link}}",
        `http://ppsbathrooms.org/verify/${unescapedEmail}/${verificationKey}`
      );
    ...
    const transporter = nodemailer.createTransport({
      ...
      secure: true,
      auth: {
        user: "ppsbathrooms@gmail.com",
        pass: emailAppPassword,
      },
    });

    const mailOptions = {
      from: "ppsbathrooms@gmail.com",
      to: unescapedEmail,
...
async function getCurrentClass(req) {
  ...
  if (req.session.authenticated) {
    const user = await userColl.findOne({
      _id: new ObjectId(req.session.profile._id),
    });
...
function getCurrentDay(schedule) {
  ...
  let adjustedWords = words.map((word) =>
    word.length > 1 ? word.toLowerCase() : word
  );
...
function getCurrentData(currentPeriod, user) {
  if (currentPeriod == -1) {
    return {
      ...
      classDescription: "No current class",
    };
  } else if (
    user.schedule[Number(currentPeriod) - 1] == undefined &&
    currentPeriod != "Lunch"
  ) {
    return {
      currentClass: "Enter your schedule to use see current period",
      classDescription: "Enter your schedule to use see current period",
    };
  } else {
    return {
      currentClass:
        currentPeriod != "Lunch"
          ? user.schedule[Number(currentPeriod) - 1]
          : "Lunch",
      classDescription:
        currentPeriod == "Lunch"
          ? "Your current class is Lunch"
          : "Your current class is in room " + user.schedule[currentPeriod - 1],
    };
  }
}
...
app.get("/account", async (req, res) => {
  ...
  if (
    ...
    !req.session.profile.emailVerified
  ) {
  ...
  if (user.access < 0) {
    ...
    if (req.session.profile.access == 2) {
      const accountJs =
        "<script>" + (await readFile("login/inserts/account.js")) + "</script>";
      const studentStyle =
        "<style>" + (await readFile("login/studentStyle.css")) + "</style>";
      const scheduleStyle =
        "<style>" + (await readFile("login/inserts/schedule.css")) + "</style>";
      const updateSchedule = await readFile(
        "login/inserts/updateSchedule.html"
      );
      const changePassword = await readFile(
        "login/inserts/changePassword.html"
      );

      const scheduleJs =
        "<script>" +
        (await readFile("login/inserts/schedule.js")) +
        "</script>";
      const passwordJs =
        "<script>" +
        (await readFile("login/inserts/updatePassword.js")) +
        "</script>";
      ...
      let dropdownOptions = `
          <option value="cleveland"${
            user.school === "cleveland" ? " selected" : ""
          }>cleveland</option>
          <option value="franklin"${
            user.school === "franklin" ? " selected" : ""
          }>franklin</option>
          <option value="ibw"${
            user.school === "ibw" ? " selected" : ""
          }>wells</option>`;
      ...
      <div id="br_prefs">
        ...
        <div class="flex_horizontal">
            <input type="checkbox" name="male" value="male" ${
              user.br_prefs.male == "true" ? "checked" : ""
            }>
            <label for="male">male</label><br>
        </div>
        <div class="flex_horizontal">
            <input type="checkbox" name="female" value="female" ${
              user.br_prefs.female == "true" ? "checked" : ""
            }>
            <label for="female">female</label><br>
        </div>
        <div class="flex_horizontal">
            <input type="checkbox" name="all" value="all" ${
              user.br_prefs.all == "true" ? "checked" : ""
            }>
      ...
      const formattedAdminData = adminData
        ...
        .map(
          (entry) =>
            `${entry.time} ${entry.value ? `| ${entry.value}` : ""} ${" | "} ${
              entry.ip
            }`
        )
        .join("<br>");
...
app.post("/createAccount", async (req, res) => {
  ...
  const existingEmail = await userColl.findOne({
  ...
  if (existingUser) {
    res.json({
      status: 0,
      error: `username '${username.toLowerCase()}' is taken`,
    });
    return;
  ...
  if (
    _usernameValid.status === true &&
    _passwordValid.status === true &&
    _emailValid.status === true &&
    _schoolValid &&
    _brPrefsValid
  ) {
    const dt = dateTime();
...
function usernameValid(username) {
  ...
  if (!usernameHasText) {
    ...
    username.replace(/\s/g, "").length < 5 ||
    username.replace(/\s/g, "").length > 24
  ) {
...
app.post("/updatePassword", async function (req, res) {
  if (req.session.authenticated) {
    if (
      req.body.passwordType == "school" &&
      hasAccess("updateSchoolPassword", req.session)
    ) {
      ...
      if (currentPass == actualPass.password) {
        ...
        await db
          .collection("data")
          .updateOne({ _id: school + "Pass" }, { $set: { password: newPass } });
      } else {
        res.json({ isCorrect: false });
      }
    } else if (
      (req.body.passwordType =
        "self" && hasAccess("updateSelfPassword", req.session))
    ) {
      const objectId = new ObjectId(req.session.profile._id);
...
function checkBrPrefsValid(prefs) {
  ...
  const keysValid =
    keys.length === 3 && keys.every((key) => validKeys.includes(key));
...
app.post("/updateSelf", async function (req, res) {
  if (req.session.authenticated && hasAccess("updateSelf", req.session)) {
      ...
      console.log(
        chalk.red(
          req.session.profile.username +
            " attempted to change their school to " +
            newValue
        )
      );
      return;
    ...
    if (
      toUpdate == "br_prefs" &&
      !checkBrPrefsValid(JSON.stringify(newValue))
    ) {
      console.log(
        chalk.red(
          req.session.profile.username +
            " attempted to change their br prefs to " +
    ...
    if (canUpdate.includes(escapeRegExp(toUpdate))) {
      if (user) {
        ...
        console.log(
          chalk.white(
            `${user.username} ${toUpdate} changed to ${JSON.stringify(
              newValue
            )}`
          )
        );
      ...
      console.log(
        chalk.red(
          user.username + " tried to update " + toUpdate + " -> " + newValue
        )
      );
    }
  }
...
app.post("/updateUser", async function (req, res) {
  if (req.session.authenticated && hasAccess("updateUser", req.session)) {
    ...
    if (user) {
      ...
      await db.collection("users").updateOne({ _id: objectId }, updateQuery);
      console.log(
        chalk.white(`${user.username} ${valueName} changed to ${newValue}`)
      );
    } else {
      console.log(chalk.red("User not found"));
    }
...
function injectDataIntoHTML(htmlContent, data, moreData) {
  ...
  data.navItems.forEach((str, index) => {
    const navItems =
      "#" + str.toLowerCase() + (index !== totalNavItems - 1 ? ", " : "");
    const navItemsFull =
      "#navbar" + str + (index !== totalNavItems - 1 ? "," : "");
  ...
  navbar += `<div id="navbar-info"><h3>users: ${
    data.users.length
  }</h3><h3>latest: ${data.users[data.users.length - 1].username}</h3></div>`;
    ...
    data.users.forEach((user) => {
      userInsert =
        `
                <div class="user">
                    <div class="userTop">
                        <p class="adminUsername" id="userUsername${user._id}">${
          user.username
        }</p>
                        <div style="display: flex; justify-content: center;">
                          <h4>access</h4>
                          <select name="access" class="userAccess" id="access${
                            user._id
                          }">
                              <option value="0" ${
                                user.access == 0 ? " selected" : ""
                              }>owner</option>
                              <option value="1" ${
                                user.access == 1 ? " selected" : ""
                              }>admin</option>
                              <option value="2" ${
                                user.access == 2 ? " selected" : ""
                              }>student</option>
                              <option value="-1" ${
                                user.access == -1 ? " selected" : ""
                              }>blocked</option>
                        ...
                        <button id="togglePerms${
                          user._id
                        }" class="clearButton">more</button>
                    </div>
        ...
        `<div>
                        <div style="display:flex;align-items:center;margin-bottom:10px">
                          <p style="margin-bottom:0px;margin-right: 10px;">email : ${
                            user.email + " "
                          }</p>
                          <img id="verificationImg" ${
                            user.emailVerified
                              ? 'src="/style/icons/check.svg" title="email verified"'
                              : 'src="/style/icons/x.svg" title="email not verified"'
                          }></img>
                        </div>
  ...
  const modifiedHTML = htmlContent
    ...
    .replace(
      "{{school_options}}",
      `<option value="cleveland"${
        moreData.user.school === "cleveland" ? " selected" : ""
      }>cleveland</option>
      <option value="franklin"${
        moreData.user.school === "franklin" ? " selected" : ""
      }>franklin</option>
      <option value="ibw"${
        moreData.user.school === "ibw" ? " selected" : ""
      }>wells</option>`
    )

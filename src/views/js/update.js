...
function buttonPressed(brNumber) {
  ...
  if (numDiff > 0 && !$("#submitButton").is(":visible")) {
    $("#submitButton").css("display", "flex").fadeIn(100);
  } else if (numDiff === 0) {
    $("#submitButton").fadeOut(100);
  }
  document.getElementById("submitButton").innerHTML =
    '<img id="icon16" src="/style/icons/check.svg"></img> <p> (' +
    numDiff +
    ")</p>";
}
...
function highlightRoom(roomNum) {
  ...
  if (roomData.hasOwnProperty("x1")) {
    document
      .getElementById("triHighlight")
      .setAttribute(
        "points",
        roomData.x1.toString() +
          "," +
          roomData.y1.toString() +
          "," +
          roomData.x2.toString() +
          "," +
          roomData.y2.toString() +
          "," +
          roomData.x3.toString() +
          "," +
          roomData.y3.toString()
      );
...
function toggleBathrooms(shown) {
  if (shown) {
    $("#svgBathrooms, #svgButtons").fadeIn(250);
  } else {
    $("#svgBathrooms, #svgButtons").fadeOut(250);
  }
}
...
function setupButtons() {
  ...
  $(document).on("click", "#highlightRoomButton", function (e) {
    if (notHighlighted) {
      ...
      notHighlighted = true;
      document.getElementById("highlightRoomButton").innerHTML =
        "<img id='icon16' src='style/icons/find.svg'>";
    ...
    switch ($("#popupId").html()) {
      ...
      case "highlight":
        ...
        if (!result) {
          ...
          removeHighlight.innerHTML =
            "<img id='icon16' src='style/icons/x.svg'>";
...
$(window).ready(function () {
  ...
  setTimeout(function () {
    $("#mainTitle").animate(
      { opacity: "1" },
      { duration: 750, easing: "swing", queue: false }
    );
  }, 50);

  setTimeout(function () {
    $("#menuSchoolButtons").animate(
      { top: "30%" },
      { duration: 650, easing: "swing", queue: false }
    );
  }, 150);
